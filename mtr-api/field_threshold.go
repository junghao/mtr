...
func (f *fieldThreshold) save(r *http.Request) *result {
	if res := checkQuery(r, []string{"localityID", "sourceID", "typeID", "lower", "upper"}, []string{}); !res.ok {
		return res
	}
	...
	if _, err = db.Exec(`INSERT INTO field.threshold(localityPK, sourcePK, typePK, lower, upper) 
		VALUES ($1,$2,$3,$4,$5)`,
		fm.localityPK, fm.sourcePK, fm.typePK, f.lower, f.upper); err != nil {
...
func (f *fieldThreshold) delete(r *http.Request) *result {
	...
	if _, err = db.Exec(`DELETE FROM field.threshold 
		...
		AND sourcePK = $2
		AND typePK = $3 `,
		fm.localityPK, fm.sourcePK, fm.typePK); err != nil {
		return internalServerError(err)
	}
